...
def get_default_labels() -> list[str]:
    return ["Crystalline", "Not Crystalline"]
...
class ModelTrainer:
    ...
    def __init__(
        ...
        self.model_params = (
            ...
            else load_model_config(model_params)
        )
        self.features = (
            features if isinstance(features, list) else load_feature_config(features)
        )
        self.targets = targets
        self.labels = list(labels) if labels is not None else get_default_labels()
        self.seed = seed
    ...
    def _get_features_targets(self):
        ...
        self.df = self.df[
            ~self.df[self.features + [self.targets]].isin([np.inf, -np.inf]).any(axis=1)
        ]  # Drop rows with inf

        self.X = self.df[self.features]
        self.y = self.df[self.targets]

    def train_test_split(self, test_size: float = None, random_state: int = None):
        """
    ...
    def update_best_run(self, run_kwargs):
    ...
    def best_model_loop(self, iterations: int = 100):
        """
...
def replace_and_clean_labels_df(
    df: pd.DataFrame,
    replace: list,
) -> pd.DataFrame:
    """
...
def train_model():
    ...
    parser.add_argument(
        "--output-path", type=str, required=True, help="Path to save the trained model"
    )
    parser.add_argument(
        "--feature-set",
        type=str,
        ...
        help="Configuration features to pull from",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default="original",
        help="Configuration to grab for model settings",
    )
    parser.add_argument(
        "--train-loops",
        type=int,
