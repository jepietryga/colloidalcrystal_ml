...
class Region():
    ...
    def __init__(self,
        ...
        self._curvature_offset = None
        self._convex_paths = None
        self._concave_paths = None
...
class PercentConvexityCurvatureFeaturizer(BaseRegionFeaturizer):
    ...
    def apply_featurizer(self, region:Region):
        ...
        # Memoization check
        if not isinstance(region._convex_paths,type(None)) and self.curvature_offset == region._curvature_offset:
            convex_paths, concave_paths = (region._convex_paths,region._concave_paths)
        else:
            convex_paths, concave_paths = find_contiguous_curvatures(edge_pixels,curvature_values,self.curvature_offset)
            region._convex_paths, region._concave_paths = (convex_paths,concave_paths)
            region._curvature_offset = self.curvature_offset
