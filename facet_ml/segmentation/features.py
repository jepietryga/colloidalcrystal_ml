...
class AverageCurvatureFeaturizer(BaseRegionFeaturizer):
    ...
    def __init__(
        self,
        min_contour_length: int = MIN_CONTOUR_LENGTH,
        window_size_ratio: float = 1 / 10,
    ):
        """
    ...
    def apply_featurizer(self, region: Region):
        ...
        Get the average curvature values for a region
        """
        edge_pixels, curvature_values = get_region_curvatures(
            region, self.min_contour_length, self.window_size_ratio
        )
...
class PercentConvexityCurvatureFeaturizer(BaseRegionFeaturizer):
    ...
    def apply_featurizer(self, region: Region):
        ...
        convex_paths, concave_paths = find_contiguous_curvatures(
            edge_pixels, curvature_values, self.curvature_offset
        )
...
class LongestContiguousConvexityCurvatureFeaturizer(BaseRegionFeaturizer):
    ...
    def apply_featurizer(self, region: Region):
        ...
        if len(convex_paths) > 0:
            ...
            longest_convex_path = []
        contiguous_convex_path_percent = (
            len(longest_convex_path) / len(edge_pixels) * 100
        )
...
class LongestContiguousConcavityCurvatureFeaturizer(BaseRegionFeaturizer):
    ...
    def apply_featurizer(self, region: Region):
        ...
        contiguous_concave_path_percent = (
            len(longest_concave_path) / len(edge_pixels) * 100
        )
...
def get_region_curvatures(
    region: Region, min_contour_length: int = 5, window_size_ratio: float = 1 / 10
):
    """
    ...
    if not isinstance(region._edge_pixels, type(None)):
        ...
        if (
            ...
            and region._window_size_ratio == window_size_ratio
        ):
            # Memoized
...
def compute_curvature(point, i, contour, window_size):
    ...
    tangent_direction_original = np.arctan2(
        np.gradient(y_neighborhood), np.gradient(x_neighborhood)
    )
    tangent_direction_original.fill(
        tangent_direction_original[len(tangent_direction_original) // 2]
    )
    ...
    rotated_x = translated_x * np.cos(
        -tangent_direction_original
    ) - translated_y * np.sin(-tangent_direction_original)
    rotated_y = translated_x * np.sin(
        -tangent_direction_original
    ) + translated_y * np.cos(-tangent_direction_original)
...
def find_contiguous_curvatures(
    edge_pixels: np.ndarray, curvature_values: np.ndarray, curvature_offset: float = 0.0
):
    """
    ...
    for convex, val in itertools.groupby(
        enumerate(edge_pixels), key=lambda x: convex_logical[x[0]]
    ):
...
def facet_score(image_segmenter):
    ...
    def row_facet_score(row: pd.Series):
        ...
        r_equivalent = np.sqrt(
            (major_axis_length**2 + minor_axis_length**2) / 2
        )  # Approximation
