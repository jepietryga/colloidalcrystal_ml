...
## Target folders
parent_image_dir_list = []
# parent_image_dir_list.append(
#     Path(__file__).parent.parent / "Images" / "Fig_3_PAE_diffusion"
# )
parent_image_dir_list.append(
    Path(__file__).parent.parent
    / "Images"
    / "20240217_Images_by_experiment"
    / "Fig3_Fig4_oven_pcr_linkerstrength"
    # / "L-2_nM-2.5_mixing-F_oven-F_embed-Ag"
)
## Load Features
...
for parent_image_dir in parent_image_dir_list:
    image_folders = [
        path
        for path in glob.glob(os.path.join(parent_image_dir, "*"))
        if os.path.isdir(path)
    ]
    print(parent_image_dir)
    print(image_folders)
